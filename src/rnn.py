...
# вспомогательные функции для оценки эффективности модели
...
# Для определения нейронной сети необходимо объявить класс,
# который наследуется от nn.Module
class RNN(nn.Module):
    # в __init__ указываем слои сети
    def __init__(
        self,
        input_size,
        hidden_size,
        ...
        **kwargs,
        ...
        self.rnn = nn.RNN(
            input_size, hidden_size, num_layers, dropout=dropout_rate, batch_first=True
        )  # batch_first means that the input must have as first dimension the batch size
        # x - > (batch_size, seq_length, input_size) (input of the model)

        self.fc = nn.Linear(
            hidden_size, n_classes
        )  # linear layer for the classification part
    ...
    def forward(self, X, **kwargs):
        ...
        """
        # torch.zeros() - создает тензор с нулевыми значениями заданной размерности
...
def train_batch(X, y, model, optimizer, criterion, device=None, **kwargs):
    """
    ...
    # Вычисление градиентов функции потерь по параметром RNN
    loss.backward()
...
def evaluate(model, dataloader, part, device=None):
    """
    model: Pytorch model
    X (batch_size, 1000, 3) : batch of examples
    y (batch_size,4): ground truth labels_train
...
# validation loss
def compute_loss(model, dataloader, criterion, device=None):
    model.eval()
    with torch.no_grad():
        val_losses = []
        for i, (x_batch, y_batch) in enumerate(dataloader):
...
def main():
    ...
    parser.add_argument(
        "-data", default=r"./data/processed/", help="Path to the dataset."
    )
    parser.add_argument(
        "-epochs", default=40, type=int, help="""Number of epochs to train the model."""
    )
    parser.add_argument(
        "-batch_size", default=512, type=int, help="Size of training batch."
    )
    ...
    parser.add_argument(
        "-path_save_model", default=r"./models/", help="Path to save the model"
    )
    ...
    configure_device(opt.device)

    with open(opt.data + "meta.pkl", "rb") as f:
        meta = pickle.load(f)
    samples = [meta["n_train"], meta["n_dev"], meta["n_test"]]
    ...
    for ii in epochs:
        ...
        for i, (X_batch, y_batch) in enumerate(train_dataloader):
            ...
            # print(i, flush=True)
            loss = train_batch(
                X_batch, y_batch, model, optimizer, criterion, device=opt.device
            )
            # input()
            del X_batch
            del y_batch
        ...
        sensitivity, specificity = evaluate(
            model, dev_dataloader, "dev", device=opt.device
        )
        val_loss = compute_loss(model, dev_dataloader, criterion, device=opt.device)
        ...
        if val_loss == np.min(valid_mean_losses):
            torch.save(
                ...
                os.path.join(
                    opt.path_save_model,
                    str(datetime.timestamp(dt)) + "model" + str(ii.item()),
                ),
            )
            ...
            torch.save(
                ...
                os.path.join(
                    opt.path_save_model,
                    str(datetime.timestamp(dt)) + "model_val" + str(ii.item()),
                ),
            )

    # Results on test set:
    matrix = evaluate(model, test_dataloader, "test", device=opt.device)
    ...
    # plot
    plot(
        (epochs, train_mean_losses),
        ylabel="Loss",
        name="training-loss-{}-{}".format(opt.learning_rate, opt.optimizer),
    )
    plot(
        (epochs, valid_specificity),
        ylabel="Specificity",
        name="validation-specificity-{}-{}".format(opt.learning_rate, opt.optimizer),
    )
    plot(
        (epochs, valid_sensitivity),
        ylabel="Sensitivity",
